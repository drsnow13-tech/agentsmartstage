...
export function Admin() {
  ...
  useEffect(() => {
    if (token) {
      api('stats', token).then(() => setLoggedIn(true)).catch(() => { setToken(''); localStorage.removeItem('ssa_admin_token'); });
    }
  }, []);
  ...
  const handleDbSetup = async () => {
    try { await api('db-setup', token); showToast('Database tables updated!'); }
    catch (e: any) { showToast(`Error: ${e.message}`); }
  };
  ...
  const handleAddCredits = async () => {
    ...
    try {
      const d = await api('add-credits', token, { email: creditEmail, credits: parseInt(creditAmount) });
      showToast(`Added ${creditAmount} credits to ${creditEmail}. New balance: ${d.newBalance}`);
      setCreditAmount(''); setCreditEmail(''); loadTab('users');
    } catch (e: any) { showToast(`Error: ${e.message}`); }
  };
  ...
  const handleTogglePromo = async (code: string, active: boolean) => {
    try { await api('toggle-promo', token, { code, active: !active }); loadTab('promos'); }
    catch (e: any) { showToast(`Error: ${e.message}`); }
  };

  const handleUpdateReport = async (id: number, status: string) => {
    try { await api('update-report', token, { id, status }); showToast(`Report #${id} marked as ${status}`); loadTab('reports'); }
    catch (e: any) { showToast(`Error: ${e.message}`); }
  };

  const handleSearchUsers = async () => { loadTab('users'); };

  const quickCredit = async (email: string, amount: number) => {
    try {
      const d = await api('add-credits', token, { email, credits: amount });
      showToast(`+${amount} credits to ${email}. Balance: ${d.newBalance}`);
      loadTab('users');
    } catch (e: any) { showToast(`Error: ${e.message}`); }
  };
          ...
          <div>
            ...
            <div style={{ display: 'flex', gap: 12, marginBottom: 20, flexWrap: 'wrap' }}>
              <div style={{ flex: 1, minWidth: 200, display: 'flex', gap: 8 }}>
                ...
                <button onClick={handleSearchUsers} style={{ padding: '10px 16px', borderRadius: 10, background: '#3b82f6', color: 'white', fontWeight: 600, border: 'none', cursor: 'pointer', fontSize: 13 }}>Search</button>
              </div>
              <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                <input value={creditEmail} onChange={e => setCreditEmail(e.target.value)} placeholder="agent@email.com"
                  style={{ width: 180, padding: '10px 14px', borderRadius: 10, border: '2px solid #334155', background: '#0f172a', color: '#f8fafc', fontSize: 14, outline: 'none' }} />
                <input value={creditAmount} onChange={e => setCreditAmount(e.target.value)} placeholder="Credits" type="number"
                ...
                <button onClick={handleAddCredits} style={{ padding: '10px 16px', borderRadius: 10, background: '#22c55e', color: 'white', fontWeight: 600, border: 'none', cursor: 'pointer', fontSize: 13, whiteSpace: 'nowrap' }}>+ Add Credits</button>
              </div>
            </div>
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
                <thead>
                  <tr style={{ borderBottom: '2px solid #334155' }}>
                    {['Email', 'Name', 'Credits', 'Last Active', 'Joined', 'Actions'].map(h => (
                      <th key={h} style={{ padding: '12px 14px', textAlign: 'left', color: '#94a3b8', fontWeight: 600, fontSize: 12, textTransform: 'uppercase', letterSpacing: '0.5px' }}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
            ...
            <p style={{ color: '#64748b', fontSize: 12, marginTop: 8 }}>{users.length} agent{users.length !== 1 ? 's' : ''} found</p>
          </div>
        )}
        ...
        {tab === 'promos' && !loading && (
          <div>
            ...
            <div style={{ background: '#1e293b', borderRadius: 12, border: '1px solid #334155', padding: 20, marginBottom: 20 }}>
              <h3 style={{ margin: '0 0 12px', fontSize: 16, fontWeight: 700 }}>Create Promo Code</h3>
              <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                <input value={newPromoCode} onChange={e => setNewPromoCode(e.target.value)} placeholder="Code (e.g. ORCHARD20)"
                  style={{ flex: 1, minWidth: 150, padding: '10px 14px', borderRadius: 10, border: '2px solid #334155', background: '#0f172a', color: '#f8fafc', fontSize: 14, outline: 'none', textTransform: 'uppercase' }} />
                <input value={newPromoCredits} onChange={e => setNewPromoCredits(e.target.value)} placeholder="Credits" type="number"
                  style={{ width: 90, padding: '10px 14px', borderRadius: 10, border: '2px solid #334155', background: '#0f172a', color: '#f8fafc', fontSize: 14, outline: 'none' }} />
                <input value={newPromoMaxUses} onChange={e => setNewPromoMaxUses(e.target.value)} placeholder="Max uses (blank=unlimited)" type="number"
                  style={{ width: 180, padding: '10px 14px', borderRadius: 10, border: '2px solid #334155', background: '#0f172a', color: '#f8fafc', fontSize: 14, outline: 'none' }} />
            ...
            <div style={{ background: '#1e293b', borderRadius: 12, border: '1px solid #334155', overflow: 'auto' }}>
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
                <thead>
                  <tr style={{ borderBottom: '2px solid #334155' }}>
                    {['Code', 'Credits', 'Used / Max', 'Expires', 'Status', 'Actions'].map(h => (
                      <th key={h} style={{ padding: '12px 14px', textAlign: 'left', color: '#94a3b8', fontWeight: 600, fontSize: 12, textTransform: 'uppercase', letterSpacing: '0.5px' }}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {promos.map(p => (
                    <tr key={p.code} style={{ borderBottom: '1px solid #1e293b' }}>
                      ...
                      <td style={{ padding: '10px 14px', color: '#94a3b8' }}>{p.expires_at ? new Date(p.expires_at).toLocaleDateString() : 'Never'}</td>
                      <td style={{ padding: '10px 14px' }}>
                      ...
                      <td style={{ padding: '10px 14px' }}>
                        <button onClick={() => handleTogglePromo(p.code, p.active)}
                          style={{ padding: '4px 10px', borderRadius: 6, background: p.active ? '#ef444422' : '#22c55e22', color: p.active ? '#ef4444' : '#22c55e', border: `1px solid ${p.active ? '#ef444444' : '#22c55e44'}`, cursor: 'pointer', fontSize: 11, fontWeight: 600 }}>
          ...
          <div>
            <div style={{ background: '#1e293b', borderRadius: 12, border: '1px solid #334155', overflow: 'auto' }}>
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
                <thead>
                  <tr style={{ borderBottom: '2px solid #334155' }}>
                    {['#', 'Agent', 'Room', 'Enhancement', 'Issue', 'Remedy', 'Status', 'Date', 'Actions'].map(h => (
                      <th key={h} style={{ padding: '12px 14px', textAlign: 'left', color: '#94a3b8', fontWeight: 600, fontSize: 12, textTransform: 'uppercase', letterSpacing: '0.5px' }}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {reports.map(r => (
                    <tr key={r.id} style={{ borderBottom: '1px solid #1e293b' }}>
                      ...
                      <td style={{ padding: '10px 14px', color: '#94a3b8' }}>{r.room_type || '—'}</td>
                      <td style={{ padding: '10px 14px', color: '#94a3b8' }}>{r.enhancement || '—'}</td>
                      ...
                      <td style={{ padding: '10px 14px' }}>
                        <span style={{ padding: '3px 10px', borderRadius: 20, fontSize: 11, fontWeight: 700,
                          background: r.status === 'pending' ? '#f9731622' : r.status === 'resolved' ? '#22c55e22' : '#3b82f622',
                          color: r.status === 'pending' ? '#f97316' : r.status === 'resolved' ? '#22c55e' : '#3b82f6' }}>{r.status}</span>
                      </td>
                      ...
                      <td style={{ padding: '10px 14px' }}>
                        <div style={{ display: 'flex', gap: 4 }}>
                          {r.status === 'pending' && (
                            <>
                              <button onClick={() => handleUpdateReport(r.id, 'in-progress')} style={{ padding: '4px 8px', borderRadius: 6, background: '#3b82f622', color: '#3b82f6', border: '1px solid #3b82f644', cursor: 'pointer', fontSize: 11, fontWeight: 600 }}>Working</button>
                              <button onClick={() => handleUpdateReport(r.id, 'resolved')} style={{ padding: '4px 8px', borderRadius: 6, background: '#22c55e22', color: '#22c55e', border: '1px solid #22c55e44', cursor: 'pointer', fontSize: 11, fontWeight: 600 }}>Resolve</button>
                            </>
                          )}
              ...
              {reports.length === 0 && <div style={{ padding: 40, textAlign: 'center', color: '#64748b' }}>No reports yet — that's a good thing!</div>}
            </div>
          </div>
        )}
      </div>

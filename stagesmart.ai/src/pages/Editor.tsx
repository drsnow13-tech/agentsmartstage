...
export function Editor() {
  ...
  useEffect(() => {
    ...
    const verified = localStorage.getItem('ssa_verified');
    if (saved && verified) { setEmail(saved); setEmailInput(saved); fetch('/api/user?email=' + encodeURIComponent(saved)).then(r => r.json()).then(d => setCredits(d.credits ?? 0)).catch(() => {}); }
  ...
  const handleEmailSubmit = async () => {
    ...
    const n = emailInput.toLowerCase().trim();
    setOtpSending(true); setOtpError('');
    try {
      const r = await fetch('/api/send-otp', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ email: n }) });
      const d = await r.json();
      if (d.sent) { setOtpSent(true); setOtpCode(''); }
      else { setOtpError(d.error || 'Failed to send code'); }
    } catch { setOtpError('Failed to send verification code'); }
  ...
  const handleVerifyOtp = async () => {
    ...
    setOtpSending(true); setOtpError('');
    try {
      const r = await fetch('/api/verify-otp', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ email: n, code: otpCode.trim() }) });
      const d = await r.json();
      if (d.verified) {
        setEmail(n); setCredits(d.credits ?? 0);
        localStorage.setItem('ssa_email', n);
        localStorage.setItem('ssa_verified', 'true');
  ...
  const handleSignOut = () => { localStorage.removeItem('ssa_email'); localStorage.removeItem('ssa_verified'); setEmail(''); setEmailInput(''); setCredits(0); setStep('upload'); setOriginalImage(null); setBase64Image(null); setTileImages([]); setTileLabels([]); setRoomType(null); setSelectedOption(null); setActiveToggles([]); setError(null); setCurrentFile(null); setHasDownloaded(false); setSelectedTile(0); setOtpSent(false); setOtpCode(''); setOtpError(''); };
          ...
          {step === 'email' && (
            <motion.div key="email" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="max-w-md mx-auto">
              ...
              <div className="bg-white rounded-2xl border border-slate-200 p-8">
                {!otpSent ? (
                  <>
                    <div className="flex items-center gap-3 mb-6"><div className="w-10 h-10 bg-orange-100 rounded-full flex items-center justify-center"><Mail className="w-5 h-5 text-orange-500" /></div><div><h2 className="font-bold text-slate-900">Enter your email</h2><p className="text-sm text-slate-500">We'll send a verification code to protect your credits</p></div></div>
                    <input type="email" value={emailInput} onChange={e => { setEmailInput(e.target.value); setOtpError(''); }} onKeyDown={e => e.key === 'Enter' && handleEmailSubmit()} placeholder="you@brokerage.com" className="w-full border-2 border-slate-200 focus:border-orange-400 rounded-xl px-4 py-3 text-slate-900 placeholder-slate-400 outline-none mb-4" autoFocus />
                    {otpError && <p className="text-red-500 text-sm mb-3">{otpError}</p>}
                    <button onClick={handleEmailSubmit} disabled={!emailInput.includes('@') || otpSending} className="w-full py-3 bg-[#1E3A8A] hover:bg-blue-900 disabled:bg-slate-300 text-white font-bold rounded-xl transition-colors">
                      {otpSending ? 'Sending code...' : 'Send Verification Code →'}
                    </button>
                    <p className="text-center text-xs text-slate-400 mt-3">One-time code sent to your email. No password needed.</p>
                  </>
                ) : (
                  <>
                    <div className="flex items-center gap-3 mb-4"><div className="w-10 h-10 bg-green-100 rounded-full flex items-center justify-center"><Mail className="w-5 h-5 text-green-500" /></div><div><h2 className="font-bold text-slate-900">Check your email</h2><p className="text-sm text-slate-500">We sent a 6-digit code to <strong>{emailInput}</strong></p></div></div>
                    <div className="bg-blue-50 rounded-xl px-4 py-3 mb-4 text-sm text-blue-700">Check your inbox and spam folder. Code expires in 10 minutes.</div>
                    <input type="text" inputMode="numeric" maxLength={6} value={otpCode} onChange={e => { setOtpCode(e.target.value.replace(/\D/g, '').slice(0, 6)); setOtpError(''); }}
                      onKeyDown={e => e.key === 'Enter' && handleVerifyOtp()}
                      placeholder="Enter 6-digit code" className="w-full border-2 border-slate-200 focus:border-orange-400 rounded-xl px-4 py-4 text-slate-900 text-center text-2xl font-black tracking-[0.3em] placeholder-slate-300 placeholder:text-base placeholder:tracking-normal outline-none mb-4" autoFocus />
                    {otpError && <p className="text-red-500 text-sm mb-3">{otpError}</p>}
                    <button onClick={handleVerifyOtp} disabled={otpCode.length < 6 || otpSending} className="w-full py-3 bg-[#1E3A8A] hover:bg-blue-900 disabled:bg-slate-300 text-white font-bold rounded-xl transition-colors mb-3">
                      {otpSending ? 'Verifying...' : 'Verify & Continue →'}
                    </button>
                    <div className="flex justify-between">
                      <button onClick={() => { setOtpSent(false); setOtpCode(''); setOtpError(''); }} className="text-sm text-slate-500 hover:text-slate-700">← Change email</button>
                      <button onClick={handleEmailSubmit} disabled={otpSending} className="text-sm text-orange-500 hover:text-orange-700 font-medium">Resend code</button>
                    </div>
                  </>
                )}
              </div>
